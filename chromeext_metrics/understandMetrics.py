#
# Based on Sample Understand Python API program (Written by Jason Quinn: 1/22/21)
#
...
# Gathering metrics for class entities
def get_class_metrics(db):
    ...
    class_kind_filter = "Class" # For fetching class-level metrics
...
# Finding entities (methods/functions) with Cyclomatic complexity and return data
def get_cyclomatic_entities(db):
    possible_kinds = ["Function", "Method", "Procedure", "Subroutine", "Task", "Constructor"]
    cyclomatic_metric_name  = ["Cyclomatic", "CountLineCode", "CountPackageCoupled"]
    results = []
    found_total = 0
    ...
    for kind_filter in possible_kinds:
        ...
        for ent in ents:
            ...
            try:
                ...
                cyclo_value = metrics.get("Cyclomatic")
                ...
                if cyclo_value is not None:
                    ...
                    entity_data['metrics'] = {
                        ...
                        for metric_name in cyclomatic_metric_name
                    }
                ...
                print(f"ERROR processing entity {ent_name}: {e}", file=sys.stderr)
                continue
        # if found_in_kind > 0:
        #    print(f"  Found {found_in_kind} entities with Cyclomatic in kind '{kind_filter}'.", file=sys.stderr)
        found_total += found_in_kind
        
    print(f"Found {found_total} total entities with function metrics.", file=sys.stderr)
    return results
...
if __name__ == '__main__':
    ...
    # ------------------- Getting Absolute Paths and Validating -------------------
    ...
    # ------------------- Understand Database Operations (create, add, analyze) -------------------
